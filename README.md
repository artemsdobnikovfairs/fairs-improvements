...
## -6. Использование устаревшей библиотеки `uuid` вместо нативного `crypto.randomUUID()` на Node.js бэкенде

- **Файл/Модули:** Сервисы и контроллеры бэкенда (`NestJS` / `Node.js`)
- **Проблема:**
  - Использование внешнего пакета `uuid` создает избыточную зависимость в `package.json`.
  - Увеличивает поверхность потенциальных уязвимостей сторонних зависимостей.
  - Нативное решение работает быстрее за счет реализации на уровне C++ внутри Node.js.
  - **Старый код:** `import { v4 as uuid } from 'uuid'; const id = uuid();`

- **Решение:**
  - Заменить внешнюю библиотеку на встроенный модуль `node:crypto`:
  - **Новый код:** `import { randomUUID } from 'node:crypto'; const id = randomUUID();`
  - *(Примечание: Начиная с Node.js v19+ функция `crypto.randomUUID()` также доступна глобально без необходимости явно делать `import`)*.
